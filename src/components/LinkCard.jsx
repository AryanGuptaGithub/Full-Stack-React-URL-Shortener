...
const LinkCard = ({url, fetchUrls}) => {
    ...
    <img src={url?.qr} alt="QR Code" className='h-32 object-contain ring ring-blue-500 self-start'  />
            ...
            <div className='flex gap-2 '>
                <Button variant='ghost' onClick={()=>navigator.clipboard.writeText(`http.trimrr.in/${url?.short_url}`)}><Copy /></Button>
                <Button variant="ghost"><Download onClick={downloadImage} /></Button>
                <Button variant="ghost" onClick={()=>fnDelete().then(()=>fetchUrls())}>{loadingDelete ? <BeatLoader size={5} color='white'/> : <Trash />}</Button>
            </div>
